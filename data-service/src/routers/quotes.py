...
async def get_quote(
    symbol: str,
    source: Optional[str] = Query(None, description="Preferred data source: futu / yahoo / tushare / akshare / longbridge"),
) -> dict[str, Any]:
    """
    获取单只股票实时行情。
...
async def get_sources_health() -> dict[str, Any]:
    ...
    Returns:
        { "ok": true, "data": {"futu": true, "yahoo": true, "tushare": false, "akshare": true, "longbridge": false} }
    """
    try:
