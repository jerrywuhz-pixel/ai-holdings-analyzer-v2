...
deploy_gateway() {
    ...
    gcloud run deploy "$GATEWAY_SERVICE" \
        ...
        --min-instances=1 \
        --max-instances=10 \
        --set-env-vars="DEPLOYMENT_MODE=cloud,OPENCLAW_DEPLOYMENT_MODE=cloud" \
        --set-secrets="SUPABASE_URL=supabase-url:latest,SUPABASE_SERVICE_ROLE_KEY=supabase-service-role-key:latest,WECHAT_APP_ID=wechat-app-id:latest,WECHAT_APP_SECRET=wechat-app-secret:latest,OPENAI_API_KEY=openai-api-key:latest" \
...
deploy_data_service() {
    ...
    gcloud run deploy "$DATA_SERVICE" \
        ...
        --min-instances=0 \
        --max-instances=5 \
        --set-secrets="SUPABASE_URL=supabase-url:latest,SUPABASE_SERVICE_ROLE_KEY=supabase-service-role-key:latest,TUSHARE_TOKEN=tushare-token:latest" \
...
main() {
    ...
    case "$target" in
        ...
        verify)
            ...
            ;;
        *)
            echo "用法: $0 --target {setup|gateway|data-service|cron|all|verify}"
            echo ""
