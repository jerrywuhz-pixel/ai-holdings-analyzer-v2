...
| `FUTU_OPEND_HOST`         | Futu OpenD 主机           | `127.0.0.1`  |
| `FUTU_OPEND_PORT`         | Futu OpenD 端口           | `11111`      |
| `FUTU_BATCH_SIZE`         | Futu 批量快照大小         | `200`        |
...
- 适合持仓分析的实时多标的刷新：一次批量快照可覆盖 CN/HK/US 多市场持仓。
- 需要本机或内网运行 Futu OpenD，并安装 Python `futu-api` SDK。
- 在 `.env` 中配置 `FUTU_OPEND_HOST`、`FUTU_OPEND_PORT`、`FUTU_BATCH_SIZE`。
- 若 SDK、OpenD 或行情权限不可用，data-service 自动回退到下方数据源。
...
1. **持仓实时行情**：优先使用 Futu OpenD 批量快照，失败时按市场回退
2. **A 股数据**：Futu 失败后使用 Tushare，再回退到 Yahoo/AkShare
3. **美股/港股**：Futu 失败后，美股回退 Yahoo Finance，港股回退 Longbridge/Yahoo
...
| 代码前缀/格式 | 首选数据源 | 回退数据源 |
|-------------|-----------|-----------|
| `SH/SZ` A 股 | Futu OpenD | Tushare → Yahoo → AkShare |
| `HK` 港股 | Futu OpenD | Longbridge → Yahoo → Tushare → AkShare |
| 美股代码 | Futu OpenD | Yahoo → Longbridge → Tushare → AkShare |
