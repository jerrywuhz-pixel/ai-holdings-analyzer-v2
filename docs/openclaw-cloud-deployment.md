...
echo -n "$FUTU_OPEND_HOST" | gcloud secrets create futu-opend-host --data-file=-
echo -n "$FUTU_OPEND_PORT" | gcloud secrets create futu-opend-port --data-file=-
...
gcloud run deploy data-service \
  ...
  --set-secrets "FUTU_OPEND_HOST=futu-opend-host:latest" \
  --set-secrets "FUTU_OPEND_PORT=futu-opend-port:latest" \
