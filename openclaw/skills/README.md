...
用户微信消息
     ...
     │
     ├─→ [Hermes Quant Options Strategy Skill] ──→ 量化/期权策略评估
     │              │
     │              ▼
...
所有量化策略与期权相关策略统一归 Hermes 执行，包括策略研究、回测、信号生成、组合权重、风控阈值、期权结构和希腊值风险评估。其他 Skills 只负责交易记录、持仓聚合、市场扫描或报告呈现；当请求涉及量化或期权判断时，应交由 `quant-options-strategy/` 的 Hermes 流程处理。
