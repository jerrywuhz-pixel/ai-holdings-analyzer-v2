...
def format_daily_report(
    ...
    # ---------- 标题 ----------
    lines.append(f"## 市场日报 - {market_name} - {report_date}")
    lines.append("")
    ...
    if strategy_suggestions:
        ...
        for suggestion in strategy_suggestions:
            ...
            lines.append(
                f"- {action_emoji} **{sym}** → {action_label} | {reason}"
            )
        ...
        lines.append("### 持仓策略建议")
        lines.append("")
        lines.append("暂无持仓，建议关注以上板块龙头股的入场机会。")
        lines.append("")

    # ---------- 页脚 ----------
    lines.append("---")
    lines.append(f"*报告生成时间：{report_date} | 数据来源：data-service*")
