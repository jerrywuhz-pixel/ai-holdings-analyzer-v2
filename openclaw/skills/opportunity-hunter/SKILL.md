...
- 若建议涉及量化回测、组合权重、仓位模型、期权结构或期权风控，必须交由 Hermes 的 `quant-options-strategy` Skill 执行；Opportunity Hunter 只负责发现机会和生成非交易性观察。
  ...
  "strategy_suggestions": [
    {
      "symbol": "SH600519",
      "action": "HOLD",
      "reason": "贵州茅台属消费板块，今日消费板块小幅上涨+0.8%，建议持有观望"
    }
  ],
  "formatted_report": "## 市场日报 - A股 - 2024-01-15\n..."
}
```
...
### 持仓策略建议
- **贵州茅台(SH600519)** → 持有观望 | 消费板块小幅上涨+0.8%，建议持有观望
- **宁德时代(SZ300750)** → 关注入场 | 新能源板块今日回调-1.2%，可关注低吸机会
```
...
| 持仓数据为空 | 策略建议部分显示"暂无持仓，建议关注以下板块龙头"，不调用持仓关联分析 |
| 所有数据源均失败 | 返回"今日市场数据暂时不可用，请稍后重试"，记录 job_runs.status = 'failed' |
...
Hermes：正在生成 A 股市场日报...
       [Market Scanner] 获取 3 只指数 + A 股涨跌分布...
       [Sector Hunter] 扫描 15 个板块 ETF...
       [Leader Selector] 识别 Top5 板块龙头...
       [Strategy Council] 生成 4 条持仓策略建议...
...
Hermes：正在生成美股市场日报...
       [Market Scanner] 获取 SPY/QQQ/DIA/IWM 行情...
       [Sector Hunter] 扫描 11 个 Sector SPDR...
       [Leader Selector] 识别 Top3 板块龙头...
       [Strategy Council] 生成 2 条持仓策略建议...
...
```
Hermes：正在生成 A 股市场日报...
       [Market Scanner] ✅ 指数行情获取成功
       [Sector Hunter] ⚠️ 部分板块ETF数据缺失（3/15 失败）
       [Leader Selector] ✅ 基于可用板块数据识别龙头
       [Strategy Council] ✅ 策略建议生成完成
