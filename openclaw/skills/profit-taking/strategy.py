...
def _format_instruction(
    ...
    if action == "TAKE_PROFIT":
        ...
        return (
            f"止盈行动：{display} 触发止盈规则，建议卖出约 {reduce_ratio:.0%}"
            f"（约 {reduce_qty} 股/份），目标价 {target_price:.2f}，动态保护价"
            f" {stop_price:.2f}。原因：{reason}"
        )
    if action == "WATCH_TARGET":
        return (
            f"止盈观察：{display} 今日可能触达止盈目标 {target_price:.2f}，"
            f"若盘中放量冲高接近目标，优先执行分批止盈；动态保护价 {stop_price:.2f}。"
            f"原因：{reason}"
        )
    return f"止盈计划：{display} 暂未触发止盈，目标价 {target_price:.2f}。原因：{reason}"
