...
#### 3.2 AI 提示词构建（Prompt Engineering）
...
- 推送后更新 `delivery_runs.status = 'delivered'`
...
| 错误场景 | 处理方式 |
|----------|----------|
| 配额不足 | 跳过 AI 生成，输出极简版；提示"AI 分析配额已用完，明日恢复" |
| OpenAI API 超时/失败 | 重试 2 次后仍失败，降级为模板化总结；记录错误到 `job_runs` |
| 当日无交易且无持仓 | 输出"暂无交易和持仓记录"，不调用 AI |
| 数据查询失败 | 回滚整个分析流程，记录 `job_runs.status = 'failed'` |
| claw 推送失败 | 更新 `delivery_runs.status = 'failed'`，标记重试；用户可通过"今日复盘"手动获取 |
...
Agent：今日无交易记录。
      ...
      [今日 AI 分析配额已用完（0/5），深度分析将于明日 15:30 自动生成。]
```
