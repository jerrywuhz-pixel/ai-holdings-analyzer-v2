...
export default async function SettingsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-6 rounded-lg bg-white p-6 shadow">
        ...
        <div className="mt-4 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">认证系统将在后续版本中启用</p>
          </div>
        </div>
      </div>
        ...
        {!heartbeat ? (
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-md border border-gray-200 p-3">
              <p className="text-xs font-medium text-gray-500">部署模式</p>
              <p className="mt-1 text-sm text-gray-900">{heartbeat.deployment_mode === 'cloud' ? '云端' : '本地'}</p>
            </div>
            <div className="rounded-md border border-gray-200 p-3">
              <p className="text-xs font-medium text-gray-500">实例 ID</p>
              <p className="mt-1 text-sm text-gray-900 font-mono">{heartbeat.instance_id}</p>
            </div>
            <div className="rounded-md border border-gray-200 p-3">
              <p className="text-xs font-medium text-gray-500">上次 Cron 执行</p>
              <p className="mt-1 text-sm text-gray-900">{timeAgo(heartbeat.last_cron_run_at)}</p>
            </div>
            <div className="rounded-md border border-gray-200 p-3">
              <p className="text-xs font-medium text-gray-500">上次心跳</p>
            ...
            {heartbeat.active_skills && heartbeat.active_skills.length > 0 && (
              <div className="col-span-full rounded-md border border-gray-200 p-3">
                <p className="text-xs font-medium text-gray-500 mb-1">活跃技能</p>
            ...
            {(heartbeat.memory_usage_mb != null || heartbeat.cpu_usage_percent != null) && (
              <div className="col-span-full rounded-md border border-gray-200 p-3">
                <p className="text-xs font-medium text-gray-500 mb-1">资源使用</p>
      ...
      {/* Data source health */}
      <div className="mb-6 rounded-lg bg-white p-6 shadow">
        <h2 className="text-lg font-medium text-gray-900">数据源健康</h2>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-4 py-2 text-right text-xs font-medium uppercase tracking-wider text-gray-500">请求</th>
                  <th className="px-4 py-2 text-right text-xs font-medium uppercase tracking-wider text-gray-500">失败</th>
      ...
      <div className="mb-6 rounded-lg bg-white p-6 shadow">
        ...
        {!quota ? (
          ...
          <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <QuotaCard label="今日 AI 调用" value={quota.daily_ai_calls} />
          </div>
        )}
